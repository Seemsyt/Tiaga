...
def get_tokenizer(model:str):
    try:
        ...
        try:
            ...
            return None
def calculate_token(text:str,model:str|None = "gpt-4")->int:
...
def truncate_text(
    ...
    current_tokens = calculate_token(text, model)
    if current_tokens <= max_tokens:
        return text

    suffix_tokens = calculate_token(suffix, model)
...
def _truncate_by_chars(text: str, target_tokens: int, suffix: str, model: str) -> str:
    ...
    while low < high:
        mid = (low + high + 1) // 2
        if calculate_token(text[:mid], model) <= target_tokens:
            low = mid
        else:
            high = mid - 1
