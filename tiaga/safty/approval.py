...
class ApprovalManager:
    def __init__(self,policy:ApprovalPolicy,cwd:Path,confirmation_callback:Callable[[ToolConfirmation],bool]|None=None)->None:
        self.policy = policy
        self.cwd = cwd
        self.confirmation_callback=confirmation_callback

    def _assert_command_safty(self,command:str):
    ...
    async def check_approval(self,context:ApprovalContext)->ApprovalDecision:
        ...
        if not context.is_mutating:
            return ApprovalDecision.APPROVED
        if context.command:
            decision = self._assert_command_safty(command=context.command)

            if decision != ApprovalDecision.NEEDS_CONFIRMATION:
                return decision
            
        for path in context.affected_paths:
            path_desicion = ApprovalDecision.NEEDS_CONFIRMATION
            if path.is_relative_to(self.cwd):
                path_desicion = ApprovalDecision.APPROVED

            else:
                return path_desicion
