...
class HookSystem:
    ...
    async def _run_command(self,command:str,timeout:float,env:dict[str,str]):
        process = await asyncio.create_subprocess_shell(command,stdout=asyncio.subprocess.PIPE,stderr=asyncio.subprocess.PIPE,cwd=self.config.cwd,env=env,start_new_session=True)
    ...
    async def trigger_before_agent(self,user_messages:str)->None:
        env = self._build_env(
        ...
        for hook in self.hooks:
            if hook.trigger == HookTrigger.BEFORE_AGENT:
                await self._run_hook(hook=hook,env=env)
